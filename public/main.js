...
(async function start(){
  ...
  const onPosition = async (pos)=>{
    ...
    if (!map){
      ...
      map = L.map('map', { center:[lat,lng], zoom:16, layers:[base.spiritStyle] });
      L.control.layers({ 'Мир духов': base.spiritStyle, 'Классическая карта': base.osm }, null, { position:'topright', collapsed:true }).addTo(map);
      ...
      await updatePlayerHeader({ username: user.first_name||user.username, level: p.level, energy: p.energy, energy_max: p.energy_max });
