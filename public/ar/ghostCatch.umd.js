(function(root,factory){
  ...
  return async function(rarity){
    if (_busy) return {success:false}; _busy=true;
    try{
      ...
      const D=(window.DIFFICULTY && window.DIFFICULTY[rarity])?window.DIFFICULTY[rarity]:{holdMs:16000, baseSpeed:200, reticleRadiusPx:60, feintEveryMs:2200};
      const circleR=D.reticleRadiusPx||60;
      let ghost={x:W/2,y:H/2,vx:0.7*D.baseSpeed/60,vy:0.55*D.baseSpeed/60};
      ...
      function tick(ts){ raf=requestAnimationFrame(tick); const dt=Math.min(32, ts-(tPrev||ts)); tPrev=ts; lastFeint+=dt; if(lastFeint>(D.feintEveryMs||2200)){ lastFeint=0; ghost.vx*=-1.12; ghost.vy*=1.08; }
        ghost.x+=ghost.vx*dt; ghost.y+=ghost.vy*dt; if(ghost.x<18||ghost.x>W-18){ ghost.vx*=-1; ghost.vx*=0.9; } if(ghost.y<18||ghost.y>H-18){ ghost.vy*=-1; ghost.vy*=0.9; }
