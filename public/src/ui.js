// Unified UI — top bar + bottom bar in one place
...
function injectStyle(){
  ...
  #ui-name{ font-weight:600; text-overflow:ellipsis; white-space:nowrap; overflow:hidden; }
  #ui-topbar .col{ display:flex; flex-direction:column; flex:1; min-width:0; }
  #ui-topbar .row{ display:flex; align-items:center; gap:10px; font-size:12px; color:var(--muted); }
  #ui-energy{ display:flex; align-items:center; gap:8px; }
  #ui-energy .bar{ height:8px; flex:1; background: rgba(255,255,255,.12); border-radius:999px; overflow:hidden; }
  #ui-energy-fill{ height:8px; width:0%; background: linear-gradient(90deg,var(--accent),var(--accent2),var(--accent3)); transition: width .3s ease; }
  #ui-energy-text{ font-variant-numeric: tabular-nums; }
  #ui-stamina{ font-variant-numeric: tabular-nums; }
  #ui-bottombar{
  ...
  body.with-ui #map{ position:absolute; top:56px; bottom:56px; left:0; right:0; }
...
export function setupUI(){
  ...
  if (!tb){
    ...
    tb.innerHTML = `
      ...
      <div class="col">
        ...
        <div class="row">
          <div id="ui-energy">
            <div class="bar"><div id="ui-energy-fill"></div></div>
            <div id="ui-energy-text">0 / 1000</div>
          </div>
          <div>•</div>
          <div>⚡ <span id="ui-stamina">20</span></div>
        </div>
      </div>
    `;
  ...
  setInterval(refreshWalletIntoTopbar, 5000);
}
